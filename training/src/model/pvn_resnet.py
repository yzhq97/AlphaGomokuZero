...
class PolicyValueNet():
    def __init__(self, board_width, board_height, log_dir=None, model_file=None, is_training=False):
        ...
        self.conv = resnet.inference(self.input_state, [32, 64, 64, 128, 256])
        ...
        self.evaluation_fc1 = tf.layers.dense(inputs=self.evaluation_conv_flat,
                                              units=22, activation=tf.nn.relu)
        ...
        if log_dir is not None:
            tf.summary.histogram('conv/activations', self.conv)
            tf.summary.scalar('conv/sparsity', tf.nn.zero_fraction(self.conv))
